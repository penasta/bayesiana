...
# Fig. 1 (duas verossimilhan¸cas para exemplo do PV)
par(mfrow=c(1,2))
...
title(main='(a)')
z<-mu-z
plot(z,f.mist,type='l',ylab='f',xlab=expression(paste('z','-',mu)),xaxt='n',lwd=2)
...
1-pbeta(theta.0,alfa,beta) # probabilidade da hip´otese alternativa
pbeta(theta.0,alfa,beta) # probabilidade da hip´otese nula
...
alfa/(alfa + beta) # m´edia da distribui¸c~ao
